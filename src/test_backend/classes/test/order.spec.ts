...
describe("Order", () => {
  ...
  it('should not checkout if cart is empty', () => {
    ...
    const shoppingCartSpy = jest.spyOn(shoppingCartMock, 'isEmpty').mockReturnValueOnce(true);
    sut.checkout();
  ...
  it('should checkout if cart is not empty', () => {
    const { sut, shoppingCartMock } = makeSut();
    const shoppingCartSpy = jest.spyOn(shoppingCartMock, 'isEmpty').mockReturnValueOnce(false);
    sut.checkout();
    expect(shoppingCartSpy).toHaveBeenCalledTimes(1);
